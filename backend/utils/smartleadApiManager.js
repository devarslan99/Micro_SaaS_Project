...
// Declare PQueue for later assignment
let PQueue; 

// Use an IIFE to dynamically import PQueue and initialize it
(async () => {
  const { default: ImportedPQueue } = await import('p-queue'); // Correct destructuring
...
const queue = () => new PQueue({ concurrency: 1 }); // Call inside function to ensure PQueue is initialized
